...
def get_certification(takeoff,student):
    ...
    # m_50_HOURS = utils.str_to_time(student[6], takeoff)
    # m_CERTIFIED = utils.str_to_time(student[5], takeoff)
    # m_PILOT_STUDENT = utils.str_to_time(student[4], takeoff)
    # m_NOVICE = utils.str_to_time(student[3], takeoff)

    # result = None

    # if m_NOVICE is None or takeoff < m_NOVICE:
    #     result = PILOT_INVALID
    # elif takeoff < m_PILOT_STUDENT or m_PILOT_STUDENT is None:
    #     result = PILOT_NOVICE
    # elif takeoff < m_CERTIFIED or m_CERTIFIED is None:
    #     result = PILOT_STUDENT
    # elif takeoff < m_50_HOURS or m_50_HOURS is None:
    ...
    for i in range(3, 9):
        if student[i]:
            timestamps.append(utils.str_to_time(student[i], takeoff))
        else:
            timestamps.append(None)

    if timestamps[0] is None or takeoff < timestamps[0]:
        return PILOT_INVALID
    elif timestamps[1] is None or takeoff < timestamps[1]:
        return PILOT_NOVICE
    elif timestamps[2] is None or takeoff < timestamps[2]:
        return PILOT_STUDENT
    elif timestamps[3] is None or takeoff < timestamps[3]:
...
def has_instrument_rating(takeoff,student):
    ...
    if student[7]:
        t = utils.str_to_time(student[7], takeoff)
    else:
        return False 
...
def has_advanced_endorsement(takeoff,student):
    ...
    Precondition: student is 10-element list of strings representing a pilot
    """
    return utils.str_to_time(student[8], takeoff) < takeoff if student[8] else False
...
def has_multiengine_endorsement(takeoff,student):
    ...
    Precondition: student is 10-element list of strings representing a pilot
    """
    return utils.str_to_time(student[9], takeoff) < takeoff if student[9] else False
...
def get_best_value(data, index, maximum=True):
    ...
    if maximum == True:
        for row in data:
            result = max(float(row[index]), result)
    else:
        for row in data:
            result = min(float(row[index]), result)
...
def get_minimums(cert, area, instructed, vfr, daytime, minimums):
    ...
    for row in minimums[1:]:
        category, conditions, table_area, table_time, ceiling, visibility, wind, crosswind = row
        if (
            (category == 'Dual' and instructed) or
            (category == 'Student' and cert >= 1 ) or 
            (category == 'Certified' and cert >= 2 ) or 
            (category == '50 Hours' and cert >= 3 )
        ) and (
            (conditions == 'VMC' and vfr) or
            (conditions == 'IMC' and not vfr)
        ) and (
            (table_area == area) or
            (table_area == 'Local' and area in ('Pattern', 'Practice Area')) or
            (table_area == 'Any')
        ) and (
            (table_time == 'Day' and daytime) or
            (table_time == 'Night' and not daytime)
        ):
            matching_rows.append((ceiling, visibility, wind, crosswind))

    if not matching_rows:
        return None

    best_ceiling = get_best_value(matching_rows, 0, False)
    best_visibility = get_best_value(matching_rows, 1, False)
    best_wind = get_best_value(matching_rows, 2, True)
    best_crosswind = get_best_value(matching_rows, 3, True)
