...
def write_csv(data,filename):
    ...
    with open(filename, 'w') as f:
        ...
        for row in data[1:]:
            format = [str(value) if not isinstance(value, datetime.datetime) else value.strftime("%Y-%m-%dT%H:%M:%S") for value in row]
            file.writerow(format) 
    pass
...
def str_to_time(timestamp,tzsource=None):
    ...
    if dt.tzinfo is None and tzsource is not None:
        if not isinstance(tzsource, datetime.datetime):
            tz = pytz.timezone(tzsource)
            result = tz.localize(dt)
        else:
            result = dt.replace(tzinfo=tzsource.tzinfo)
        return result
    else:
        return dt
...
def daytime(time,daycycle):
    ...
    new_time = time
    tz_zone = pytz.timezone(daycycle['timezone'])

    if time.tzinfo is None:
        new_time = tz_zone.localize(time)
    time_str = new_time.isoformat()

    try:
        sunrise = parse(daycycle[time_str[:4]][time_str[5:10]]['sunrise'])
        sunset = parse(daycycle[time_str[:4]][time_str[5:10]]['sunset'])
        date_sunrise = tz_zone.localize(datetime.datetime.combine(new_time.date(), sunrise.time()))
        date_sunset = tz_zone.localize(datetime.datetime.combine(new_time.date(), sunset.time()))
        return date_sunrise < new_time < date_sunset
    except KeyError:
        return None
    pass
