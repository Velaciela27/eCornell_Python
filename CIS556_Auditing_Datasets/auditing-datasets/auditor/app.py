...
def discover_violations(directory,output):
    ...
    if output is not None:
        row_0 = [['STUDENT','AIRPLANE','INSTRUCTOR','TAKEOFF','LANDING','FILED','AREA','REASON']]
        utils.write_csv(row_0 + result, output)
    ...
    if len(result) == 0:
        ...
        print(f'{len(result)} violations found.')
...
def execute(args):
    ...
    if len(args) > 2 or len(args) == 0:
        ...
        try:
            discover_violations(*tuple(args))
        except:
            print('Usage: python auditor dataset [output.csv]')
