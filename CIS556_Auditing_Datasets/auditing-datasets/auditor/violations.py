...
def bad_visibility(visibility,minimum):
    ...
    table_visibility = visibility.get('minimum', None)
    if table_visibility is None: 
        table_visibility = visibility['prevailing']
    
    if visibility['units'] == 'FT':
        table_visibility = float(table_visibility)/5280
    
    return float(table_visibility) < minimum
...
def bad_winds(winds,maxwind,maxcross):
    ...
    table_speed = winds.get('speed', None)
    table_crosswind = winds.get('crosswind', None)
    table_gusts = winds.get('gusts', None)

    if table_gusts is not None:
        table_speed = max(table_gusts, table_speed)
    
    if winds['units'] == 'MPS':
        table_speed = float(table_speed) * 1.94384
        table_crosswind = float(table_crosswind) * 1.94384
        return table_speed > maxwind or table_crosswind > maxcross
    else:
        table_speed = float(table_speed)
        table_crosswind = float(table_crosswind)
        return table_speed > maxwind or table_crosswind > maxcross
...
def bad_ceiling(ceiling,minimum):
    ...
    for dic in ceiling:
        table_type = dic.get('type', None)
        table_height = float(dic.get('height', None))
        if table_type in ('broken', 'overcast', 'indefinite ceiling'):
            height_list.append(table_height)

    if len(height_list) == 0:
        return False
    else:
        max_height = min(height_list)
        return max_height < minimum
...
def get_weather_report(takeoff,weather):
    ...
    dt = takeoff.isoformat()

    result = weather.get(dt, None)

    if result is None:
        keys_list = list(weather.keys())
        key_dt = max([parse(x) for x in keys_list if x < dt])
        result = weather.get(key_dt.isoformat(), None)
    return result
...
def get_weather_violation(weather,minimums):
    ...
    dic_visibility = weather.get('visibility', None)
    dic_wind = weather.get('wind', None) 
    dic_ceiling = weather.get('sky', None)

    result = ''

    if bad_visibility(dic_visibility, minimums[1]):
        result += 'Visibility'
    
    if bad_winds(dic_wind, minimums[2], minimums[3]):
        result += 'Winds'
    
    if bad_ceiling(dic_ceiling, minimums[0]):
        result += 'Ceiling'
...
def list_weather_violations(directory):
    ...
    daycycle_dic = utils.read_json(os.path.join(directory, DAYCYCLE))
    weather_dic = utils.read_json(os.path.join(directory, WEATHER))
    minimums_list = utils.read_csv(os.path.join(directory, MINIMUMS))
    student_list = utils.read_csv(os.path.join(directory, STUDENTS))
    lessons_list = utils.read_csv(os.path.join(directory, LESSONS))

    result = []


    # For each of the lessons
    for row in lessons_list[1:]:
        # Get the takeoff time
        takeoff_time = utils.str_to_time(row[3])
        student = utils.get_for_id(row[0], student_list)

        daytime_ind = utils.daytime(takeoff_time, daycycle_dic)
        ...
        credentials = pilots.get_certification(takeoff_time, student)
        # Get the pilot minimums
        pilot_minimums = pilots.get_minimums(credentials, row[6], instrutor, filed, daytime_ind, minimums_list)
        # Get the weather conditions
        weather_conditions = get_weather_report(takeoff_time, weather_dic)
        # Check for a violation and add it to the list if so
        violation = get_weather_violation(weather_conditions, pilot_minimums)
